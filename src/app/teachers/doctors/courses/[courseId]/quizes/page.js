  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      ...
      try {
        ...
        showQuizBlockMessage('Failed to load quizzes. Please try again later.', 'error');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSaveChangesQuiz = async () => {
    ...
    try {
      ...
      for (const file of newFiles) {
        await uploadQuizFile(editedQuiz.id, {
          id: file.id,
          name: file.name,
          url: file.url
        });
  ...
  const handleSaveNewQuiz = async () => {
    try {
      ...
      for (const file of files) {
        await uploadQuizFile(createdQuiz.id, {
          id: file.id,
          name: file.name,
          url: file.url
        });
      ...
      // Actualizar lista de quizzes
      setAllQuizzes(prev => [...prev, createdQuiz]);
