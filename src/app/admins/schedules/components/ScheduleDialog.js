...
export default function ScheduleDialog({
  ...
  // Time period management
  const [isAddingTimePeriod, setIsAddingTimePeriod] = useState(false);
  const [newTimePeriodStart, setNewTimePeriodStart] = useState('08:00');
  const [newTimePeriodEnd, setNewTimePeriodEnd] = useState('09:30');
  ...
  const handleCellSelection = (day, periodIndex, department, group, mode = 'weekly') => {
    // Call the hook's function to set the selectedCell for data operations
    // and get existing session data.
    const existingSessionData = handleCellDoubleClick(day, periodIndex, department, group, mode);

    // Set local state to show and populate the form
    setSelectedSession({ day, periodIndex, department, group, mode }); // Activates the form UI
        ...
        <div className="form-row">
          ...
          <label>
            ...
            <input
              ...
              onChange={handleSessionFormChange}
              placeholder="e.g. Expert Systems"
            />
          </label>
        </div>
            ...
            <div className="session-type-options">
              <label className="radio-label">
                <input
                  ...
                  name="isLecture"
                  checked={sessionFormData.isLecture}
                  onChange={() => setSessionFormData({...sessionFormData, isLecture: true})}
                />
                <div className="color-preview lecture"></div>
              ...
              <label className="radio-label">
                <input
                  ...
                  name="isLecture"
                  checked={!sessionFormData.isLecture}
                  onChange={() => setSessionFormData({...sessionFormData, isLecture: false})}
                />
                <div className="color-preview section"></div>
                Section (with assistant)
              </label>
            </div>
          </label>
        </div>
  ...
  const renderWeeklyTable = () => {
    ...
    // Track which cells are covered by a spanned cell
    const coveredCells = {};

    // Mark cells covered by spanned sessions
    days.forEach(day => {
      enabledTimePeriods.forEach((period, periodIndex) => {
        enabledGroups.forEach(group => {
          const session = getCellSession(day, periodIndex, 'ALL', group.name, 'weekly');
          if (session && (session.colspan > 1 || session.rowspan > 1)) {
            const baseCellKey = `${day}-${periodIndex}-ALL-${group.name}`;
            coveredCells[baseCellKey] = { isMainCell: true, session };

            if (session.rowspan > 1) {
              const groupStartIndex = enabledGroups.findIndex(g => g.name === group.name);
              for (let i = 1; i < session.rowspan; i++) {
                const targetGroupIndex = groupStartIndex + i;
                if (targetGroupIndex < enabledGroups.length) {
                  const targetGroup = enabledGroups[targetGroupIndex];
                  const rowCellKey = `${day}-${periodIndex}-ALL-${targetGroup.name}`;
                  coveredCells[rowCellKey] = { isMainCell: false, coveredBy: baseCellKey };
                }
              }
            }
            ...
            if (session.colspan > 1) {
              for (let i = 1; i < session.colspan; i++) {
                const targetPeriodIndex = periodIndex + i;
                if (targetPeriodIndex < enabledTimePeriods.length) {
                  const colCellKey = `${day}-${targetPeriodIndex}-ALL-${group.name}`;
                  coveredCells[colCellKey] = { isMainCell: false, coveredBy: baseCellKey };
                }
              }
            }
          }
            ...
            enabledGroups.map((group, groupIndex) => {
              ...
              return (
                <tr key={`${day}-${group.name}`}>
                  ...
                  <td className="group-cell">{group.name}</td>
                  {enabledTimePeriods.map((period, periodIndex) => {
                    const cellKeyForRender = `${day}-${periodIndex}-ALL-${group.name}`;
                    const cellInfoForRender = coveredCells[cellKeyForRender];
                    
                    if (cellInfoForRender && !cellInfoForRender.isMainCell) {
                      return null;
                    }
                    
                    const sessionDataForCell = getCellSession(day, periodIndex, 'ALL', group.name, 'weekly');
                    const isSelected = resizingCell && 
                      resizingCell.day === day && 
                      resizingCell.periodIndex === periodIndex && 
                      resizingCell.group === group.name;
                    ...
                    return (
                      <td 
                        key={`${day}-${group.name}-${period.id}`}
                        className="schedule-cell table-drop-target"
                        onClick={() => handleCellClick(day, periodIndex, 'ALL', group.name)}
                        onDoubleClick={() => handleCellSelection(day, periodIndex, 'ALL', group.name, 'weekly')}
                        colSpan={colSpan}
                        rowSpan={rowSpan}
                        onDragOver={(event) => {
                          event.preventDefault();
