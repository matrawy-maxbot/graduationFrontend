...
export default function RegistrationManager({ studentData, currentSemester, registrationSettings }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const availableResponse = await axios.get(`http://localhost:3001/api/v1/students/me/available-courses?_t=${timestamp}`, {
          headers: {
        ...
        const availableCourseData = availableResponse.data.data || availableResponse.data || [];
        ...
        try {
          const registeredResponse = await axios.get(`http://localhost:3001/api/v1/course-registers/me/courses/${currentSemester.id}`, {
            headers: {
              'Authorization': `Bearer ${accessToken}`,
              // 'Cache-Control': 'no-cache, no-store, must-revalidate',
              // 'Pragma': 'no-cache',
          ...
          const registeredCourseData = registeredResponse.data.data || registeredResponse.data || [];
  ...
  const confirmUnregister = async () => {
    ...
    try {
      ...
      // Send delete request to server using student-specific endpoint
      await axios.delete(
        `http://localhost:3001/api/v1/course-registers/me/${courseToUnregister.registrationId}?_t=${timestamp}`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
            // 'Cache-Control': 'no-cache, no-store, must-revalidate',
      ...
      setUnregisterError(err.response?.data?.message || 'فشل في إلغاء تسجيل المقرر. يرجى المحاولة مرة أخرى.');
    } finally {
  ...
  const handleConfirm = async () => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:3001/api/v1/course-registers/register?_t=${timestamp}`, 
        registrationData,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
      ...
      setRegistrationError(err.response?.data?.message || 'Failed to register courses. Please try again.');
      setIsConfirmDialogOpen(false);
    }
  };
